...
export class ZkTurkClient {
  ...
  async startProblem(problemId: number, signal = '', root = 123, nullifierHash = 456, proof = [1, 2, 3, 4, 5, 6, 7, 8]) {
    const fee = await this._getStakeValue();
    const options = { value: fee };
    const signerAddress = await this.signer.getAddress();
    return await this.contract.joinProblem(problemId, signerAddress, root, nullifierHash, proof, options);
  }
  ...
  async createProblem(
    ...
    return await this.contract.addProblem(title, description, urlToTasks, answers, workersMax, taskPriceWei, answersMax, options);
  }
  ...
  async withdraw(problemId: number, taskAnswerIds: number[], answers: string[], seed = DEFAULT_SEED) {
    // Convert seed.
    const seedBytes = stringToBytes32(seed);
    const signerAddress = await this.signer.getAddress();
    await this.contract.withdrawAndDecipher(signerAddress, problemId, taskAnswerIds, answers, seedBytes);
  }
