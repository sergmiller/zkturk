...
contract ZkTurkContract is Ownable, Crypto {
    ...
    mapping(address => uint) public workerToProblem; // TODO: to problems (MVP - 1 worker could solve simultaniously only 1 problem)
    ...
    function setUseVerification(bool _state) onlyOwner external {
        useVerification = _state;
    }

    function getProblem(uint problemId) public view returns (Problem memory) {
    ...
    // function getWorkerToProblemAnswers(address worker, uint problemId) public view returns ([] memory) {
    //     return problems;
    // }
    ...
    function withdrawAndForget() external {
        uint activeProblemId = workerToProblem[msg.sender];
        if (activeProblemId != 0) {
            delete workerToProblem[msg.sender];
            delete workerToStakeId[msg.sender];
            problemWorkersCounts[activeProblemId] -= 1;
        }
    }
    ...
    */
    function withdrawAndDecipher(
        ...
        for (uint i=0; i < taskIds.length; i++) {
            string memory composedSeed = string.concat(answers[i], seedPhrase);

            TaskAnswer storage taskAnswer = taskAnswers[taskIds[i]];
            if (compareStrings(taskAnswer.answer, NO_KEY_FLAG)) {
                // TODO: try/except.
                assertInvalidSignature(worker, composedSeed, taskAnswer.cipheredAnswer);
                require(isAnswerAllowed(answers[i], problemId), "Answer is not allowed.");

                taskAnswer.answer = answers[i];
