...
class ContractABC {
    constructor(web3, address="0xD9245acA14c7E1985e8E16CB987Cd11C7b485c53") {  // def to polygon.
...
export class TurkContractClient extends ContractABC {
    ...
    _parseArrayToModel(arrayToParse, model) {
        ...
        for (let i=0; i < arrayToParse.length; i++) {
            data.push(new model(...arrayToParse[i]))
        }
        return data
    }
    ...
    async getAvailableProblems() {
        ...
        let res = await this._callMethod(requestConfig, 'call') || []
        if (res) {
            // ignore 0 problem (read contract).
            res = res.slice(1)
        }
        return this._parseArrayToModel(res, ProblemModel)
    }
