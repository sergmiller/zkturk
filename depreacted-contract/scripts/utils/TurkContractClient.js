...
class ContractABC {
  ...
  async _sendMethodImpl(method, args, value, userAddress = null) {
    ...
    console.log("depreacted-contract address is " + this.address);
  ...
  async _callMethodImpl(method, args, value, userAddress = null) {
    console.log("Calling depreacted-contract instead of tx sending...");
    const [sender] = await getWeb3Accounts(this.web3, userAddress);
    console.log("sender is " + sender);
    console.log("method is " + method);
    let methodFunc = await this._buildMethod(method, args);
...
export class TurkContractClient extends ContractABC {
  ...
  // Business paid for the problem a little fee described in depreacted-contract.
  ...
  async getAllTasks(problemId) {
      ...
      const allTasks = this._parseArrayToModel(problem.urlToTask, TaskOptionModel)
      if (!allTasks) {
        return []
      }
      ...
      for (let i = 0; i < allTasks.length; i++) {
        ...
        const isTaskNotSolved = await this.isTaskNotAnsweredByWorker(problemId, task.id)
        if (!isTaskNotSolved) {
          continue;
        }
        data.push(task);
